...
MAXHOLESIZE = 1000
EPSILON = 0.2
REMESH_TARGET = ml.PercentageValue(0.6)
...
def mesh_fix(ms, dirname, meshname):
    ...
    meshfix.save(os.path.join(tmp_dir, "{}_fixed.ply".format(meshname)))

def remesh(ms, dirname, meshname, targetlen):
    ms.load_new_mesh("{}/tmp/{}_fixed.ply".format(dirname, meshname))
    # [original, fixed]

    ms.apply_filter("meshing_isotropic_explicit_remeshing", targetlen=targetlen)
    # [original, remeshed]

    ms.save_current_mesh("{}/tmp/{}_remeshed.obj".format(dirname, meshname))

def normalize(ms):
    ms.apply_filter("compute_matrix_from_scaling_or_normalization", scalecenter="barycenter", unitflag=True, alllayers=True)
    ms.apply_filter("compute_matrix_from_translation", traslmethod="Center on Layer BBox", alllayers=True)
...
def write_mask(ms, epsilon, dirname, meshname):
    ...
    ms.apply_filter("compute_scalar_by_distance_from_another_mesh_per_vertex", measuremesh=ms.current_mesh_id(), refmesh=0, signeddist=False)
...
def smooth(ms, dirname, meshname):
    ...
    ms.apply_filter("apply_coord_laplacian_smoothing", selected=True)
    ms.apply_filter("apply_coord_laplacian_smoothing", stepsmoothnum=SMOOTH_ITER, cotangentweight=False, selected=False)
...
def main():
    ...
    filename = args.input
    targetlen = ml.PercentageValue(args.remesh)
